...
def score_project_paper(project: dict[str, Any], paper: dict[str, Any]) -> tuple[float, list[str]]:
    project_text = _normalize(build_project_text(project))
    paper_text = _normalize(build_paper_text(paper))
    primary_paper_text = _normalize(" ".join([str(paper.get("title", "")), str(paper.get("abstract", ""))]))
...
def match_project_papers(
    ...
    *,
    ...
    project_rows = [validate_instance(project, "project_profile.schema.json") for project in projects]
    paper_rows = [validate_instance(paper, "paper_profile.schema.json") for paper in papers]
    ...
    try:
        for project in project_rows:
            ...
            for paper in paper_rows:
                if conn is not None:
                    ...
                    if not decision.should_process:
                        skipped_pairs += 1
                        warnings.append(f"skipped unchanged pair: {project['project_id']} -> {paper['citekey']}")
                        continue
            ...
            for rank, candidate in enumerate(sorted(scored, key=_candidate_sort_key)[:top_n], start=1):
                candidate["rank"] = rank
                candidates.append(validate_instance(candidate, "project_paper_match.schema.json"))
    finally:
        if conn is not None:
            conn.close()
...
def write_candidates_jsonl(output_path: str | Path, candidates: Iterable[dict[str, Any]]) -> Path:
    path = Path(output_path)
    validated = [validate_instance(candidate, "project_paper_match.schema.json") for candidate in candidates]
...
def _candidate_sort_key(candidate: dict[str, Any]) -> tuple[float, str]:
    return (-float(candidate["candidate_score"]), str(candidate["citekey"]))
